...
#define RSP_PLUGIN_API_VERSION 0x0102
...
EXPORT u32 CALL API_PREFIX(DoRspCycles)(u32 cycles)
{
    ...
    if (task_type < M_GFXTASK || task_type > M_HVQMTASK)
    {
        GET_RCP_REG(SP_STATUS_REG) |= SP_STATUS_HALT;
        return cycles;
    }
    switch (task_type) {
    ...
    case M_VIDTASK:
        ...
        break;
    case M_NJPEGTASK:
        GET_RCP_REG(SP_STATUS_REG) |= SP_STATUS_SIG2 | SP_STATUS_BROKE | SP_STATUS_HALT;
        if (GET_RCP_REG(SP_STATUS_REG) & SP_STATUS_INTR_BREAK) {
            GET_RCP_REG(MI_INTR_REG) |= 0x00000001;
            GET_RSP_INFO(CheckInterrupts)();
        }
