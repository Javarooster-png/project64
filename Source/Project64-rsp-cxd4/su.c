...
static INLINE pu8 SP_MEM_PTR(unsigned int address)
{
    address &= 0x00001FFFu;
    return (address & 0x1000u) ? (IMEM + (address & 0x0FFFu)) : (DMEM + address);
}
...
void SP_DMA_READ(void)
{
    ...
    do {
        ...
        do {
            ...
            *(pi64)SP_MEM_PTR(offC) = (offD & ~MAX_DRAM_DMA_ADDR) ? 0 : *(pi64)(DRAM + offD);
            i += 0x008;
        } while (i < length);
    } while (count);
...
void SP_DMA_WRITE(void)
{
    ...
    do {
        ...
        do {
            offC = (count*length + *CR[0x0] + i) & 0x00001FF8ul;
            offD = (count*skip + *CR[0x1] + i) & 0x00FFFFF8ul;
            if ((offD & ~MAX_DRAM_DMA_ADDR) == 0)
            {
                *(pi64)(DRAM + offD) = *(pi64)SP_MEM_PTR(offC);
            }
            i += 0x000008;
        } while (i < length);
    } while (count);
